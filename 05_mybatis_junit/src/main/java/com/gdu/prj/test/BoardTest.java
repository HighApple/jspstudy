...
public class BoardTest {
 ...
 // @Test 주석처리. test 목록에서 제외
  public void 목록개수테스트() {
    
    int total = boardDao.getBoardCount();
    assertEquals(1000, total);   // total == 1000 이면 테스트 성공   // arg -> 기대 == 실제 (성공(녹색)/실패(빨강색))
  }
 ...
 // @Test
  public void 등록테스트() {
    ...
    // 데이터 1,000 개가 있는 상태에서 이걸 삽입하면 시퀀스가 돌아가면서 board_no 1이 2개가 생겨 pk를 위반
    // -> 기존 데이터 삭제하고 진행.
    // DB에 테스트코드가 들어와있는걸 확인 가능. 
