...
public class MyRequest extends HttpServlet {
   ...
   //   2) 파라미터가 없으면 null 이다.
	 //   3) 파라미터의 값이 없으면 ""(빈 문자열) 이다. 
	  ...
	  String strNumber = request.getParameter("number"); //getParameter 가 요청 받음
	  int number = 0; 
	  if(strNumber != null && !strNumber.isEmpty())    //isEmpty로 빈문자열처리
	     number = Integer.parseInt(strNumber);
	  System.out.println(number);
	  
	  /* Optional<T> 클래스를 이용한 null&빈문자열 처리 */ //활용 다수.
    ...
    System.out.println(number2);
	  // -> 만약 strNumber2가 null 이라면 0. 삼항연산자로 빈문자열처리
